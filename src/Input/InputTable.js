...
let [items,setItems]=useState([{ product: { name: '', price: 10 }, quantity: '' }]);
const products = [    { name: 'Sugar', price: 80 },    { name: 'Oats', price: 100 },    { name: 'Coffee', price: 60 },    { name: 'Tea', price: 50 }  ];
...
const handleAddItem = () => {
  const newItems = [...items, { product: { name: '', price: 10 }, quantity: '' }];
  setItems(newItems);
};
      ...
      {items.map((item, index) => (
        ...
        <div className={styles.mainDivOfForm} key={index}>
          ...
          <div className={styles.first}>
            Product:
            <select className={styles.product} value={item.product.name} onChange={(e) => handleProductChange(index, e)}>
