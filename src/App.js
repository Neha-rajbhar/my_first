...
function App() {
  ...
  function setData(takeValue){
    console.log("i am parent");
    alert(takeValue.color);
      ...
      <div className='mainDiv'>
        {(check && ((finalResult >= 12) ? <h1>Congratulation {finalResult * 2}/20</h1> :
          <div className='failedDiv'>
            <h1 className='headings'> Congratulation {finalResult * 2}/20</h1>
            <button onClick={handleRestart} className="bttn">Restart</button>
          </div>))}
        {
          question.map((x, index) => {
            return (<div key={index}>
              ...
              {(x.Flag != 10) ? <button onClick={() => handleNext(qnaId)} className="btn">Next</button> : <button onClick={handleSubmit1} className="btn">Submit</button>}
            </div>
            )
          })
        }
